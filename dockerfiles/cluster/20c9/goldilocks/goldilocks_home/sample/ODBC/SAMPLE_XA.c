...
/* Update funtion */
SQLRETURN testUpdate( SQLHDBC aDbc )
{
    SQLHSTMT    sStmt                   = NULL;
    SQLINTEGER  sState                  = 0;
    SQLLEN      sCount                  = 0;
    ...
    switch( sReturn )
    {
        ...
        case SQL_SUCCESS_WITH_INFO:
            PrintDiagnosticRecord( SQL_HANDLE_STMT, sStmt );
            break;
        case SQL_NO_DATA: /* Data is not updated. */
            break;
        case SQL_NEED_DATA:
            /* FALL-THROUGH */ /* Error message is output. */
        case SQL_STILL_EXECUTING:
...
int main( int aArgc, char** aArgv )
{
    ...
    sDbc = SQLGetXaConnectionHandle(NULL);
    ...
    /* If you SQL_SUCCESS that is Update funtion success */ 
