...
{
    SQLRETURN    rc;
    SQLCHAR      Col1[ROW_ARRAY_SIZE][11];
    SQLINTEGER   Col2[ROW_ARRAY_SIZE];
    SQLLEN       ColInd1[ROW_ARRAY_SIZE];
    SQLLEN       ColInd2[ROW_ARRAY_SIZE];
    int          i;
    ...
    rc = SQLExecDirect( gStmt, 
                        (SQLCHAR*)"DROP TABLE IF EXISTS TCOLUMN_WISE",
                        SQL_NTS ); 
    CHECK_HANDLE( SQL_HANDLE_STMT, gStmt, rc );

    rc = SQLExecDirect( gStmt, 
                        (SQLCHAR*)"CREATE TABLE TCOLUMN_WISE ( C1 VARCHAR(10), C2 INTEGER PRIMARY KEY )",
                        SQL_NTS ); 
    CHECK_HANDLE( SQL_HANDLE_STMT, gStmt, rc );
    ...
    rc = SQLSetStmtAttr( gStmt, SQL_ATTR_PARAM_BIND_TYPE, (SQLPOINTER)SQL_BIND_BY_COLUMN, 0 );
    CHECK_HANDLE( SQL_HANDLE_STMT, gStmt, rc );
    ...
    rc = SQLSetStmtAttr( gStmt, SQL_ATTR_ATOMIC_EXECUTION, (SQLPOINTER)SQL_ATOMIC_EXECUTION_ON, 0 );
    CHECK_HANDLE( SQL_HANDLE_STMT, gStmt, rc );
    ...
    rc = SQLPrepare( gStmt, (SQLCHAR*)"INSERT INTO TCOLUMN_WISE VALUES ( ?, ? )", SQL_NTS );
    CHECK_HANDLE( SQL_HANDLE_STMT, gStmt, rc );

    for( i = 0; i < ROW_ARRAY_SIZE; i++ )
    {
        memset( Col1[i], 'A' + i, 10 );
    ...
    rc = SQLSetStmtAttr( gStmt, SQL_ATTR_PARAMSET_SIZE, (SQLPOINTER)4, 0 );
    CHECK_HANDLE( SQL_HANDLE_STMT, gStmt, rc );

    for( i = 0; i < 4; i++ )
    {
        memset( Col1[i], 'A' + ROW_ARRAY_SIZE + i, 10 );
        
        Col1[i][10] = 0;
        ColInd1[i] = SQL_NTS;
        
        Col2[i]    = ROW_ARRAY_SIZE + i;
        ColInd2[i] = 0;
    }
    ...
    rc = SQLExecDirect( gStmt, 
                        (SQLCHAR*)"SELECT * FROM TCOLUMN_WISE",
                        SQL_NTS ); 
    CHECK_HANDLE( SQL_HANDLE_STMT, gStmt, rc );
