...
int main( int argc, char ** argv )
{
    ...
    rc = SQLExecDirect( gStmt, 
                        (SQLCHAR*)"INSERT INTO TCOLUMN_WISE VALUES ('AAAAAAAAAA', 1),"
                                                                  "('BBBBBBBBBB', 2),"
                                                                  "('CCCCCCCCCC', 3),"
                                                                  "('DDDDDDDDDD', 4),"
                                                                  "('EEEEEEEEEE', 5),"
    ...
    rc = SQLSetStmtAttr( gStmt, SQL_ATTR_ROW_BIND_TYPE, (SQLPOINTER)SQL_BIND_BY_COLUMN, 0 );
    CHECK_HANDLE( SQL_HANDLE_STMT, gStmt, rc );
    ...
    rc = SQLBindCol( gStmt, 1, SQL_C_CHAR, gCol1, sizeof( gCol1[0] ), gColInd1 );
    CHECK_HANDLE( SQL_HANDLE_STMT, gStmt, rc );
                             
    rc = SQLBindCol( gStmt, 2, SQL_C_SLONG, gCol2, 0, gColInd2 );
    CHECK_HANDLE( SQL_HANDLE_STMT, gStmt, rc );

    rc = SQLExecDirect( gStmt, (SQLCHAR*)"SELECT * FROM TCOLUMN_WISE", SQL_NTS );
    CHECK_HANDLE( SQL_HANDLE_STMT, gStmt, rc );
