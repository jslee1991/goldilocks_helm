...
static void free_cols( pdo_stmt_t          * stmt,
                       pdo_goldilocks_stmt * S TSRMLS_DC )
{
    int i;
...
static int goldilocks_stmt_dtor( pdo_stmt_t * stmt TSRMLS_DC )
{
...
static int goldilocks_stmt_execute( pdo_stmt_t *stmt TSRMLS_DC )
{
    SQLRETURN rc;
    pdo_goldilocks_stmt *S = (pdo_goldilocks_stmt*)stmt->driver_data;
    char *buf = NULL;
            ...
            case PDO_PARAM_EVT_ALLOC:
                {
                    ...
                    switch( param_type )
                    {
                        case PDO_PARAM_BOOL:
                            ...
                            sqltype = SQL_BOOLEAN;
                            data    = (SQLPOINTER)(&Z_LVAL_P(param->parameter));
                            break;
                        case PDO_PARAM_INT:
                            if( sizeof(long) == 4 )
                            {
            ...
            case PDO_PARAM_EVT_EXEC_PRE:
                ...
                else
                {
                    if( param_type == PDO_PARAM_STR )
                    {
                        ...
                        P->strbuf.arr = (SQLCHAR*)Z_STRVAL_P(param->parameter);
                    ...
                    else
                    {
                        switch( param_type )
                        {
                            case PDO_PARAM_STR :
                                ...
                                if( Z_STRLEN_P(param->parameter) > P->len + 1 )
                                {
                                    memcpy( Z_STRVAL_P(param->parameter), P->outbuf, P->len );
                                }
                                else
                                {
                                    Z_STRVAL_P(param->parameter) = str_erealloc( Z_STRVAL_P(param->parameter),
                                                                                 P->len + 1 );
                                    memcpy( Z_STRVAL_P(param->parameter), P->outbuf, P->len );
                            ...
                            case PDO_PARAM_LOB :
                                Z_TYPE_P(param->parameter) = IS_STRING;
                                if( Z_STRLEN_P(param->parameter) > P->len )
                                {
                                    memcpy( Z_STRVAL_P(param->parameter), P->outbuf, P->len );
                                }
                                else
                                            ...
                                            zval       * return_value TSRMLS_DC )
{
    ...
    {
        case SQL_BOOLEAN :
            add_assoc_string( return_value, "native_type", "boolean", 1 );
            break;
        case SQL_REAL :
        case SQL_DOUBLE :
        case SQL_FLOAT :
    ...
    add_assoc_string( return_value, "goldilocks:decl_type", decl_type, 1 );
    add_assoc_zval( return_value, "flags", flags );
    add_assoc_string( return_value, "table", table, 1 );
    
    return 1;
}

static int goldilocks_stmt_next_rowset( pdo_stmt_t *stmt TSRMLS_DC )
{
    return 0;
}

static int pdo_goldilocks_stmt_cursor_closer( pdo_stmt_t *stmt TSRMLS_DC )
{
    pdo_goldilocks_stmt *S = (pdo_goldilocks_stmt*)stmt->driver_data;
