...
# glocator 
GLOCATOR_MASTER_NAME=$APP_NAME'-0'
...
### glocator add member ###
MASTER_DNS=`cat /home/sunje/nfs_volumn/$GLOCATOR_MASTER_NAME`
gloctl -i $MASTER_DNS -p 42581 << EOF
ADD MEMBER "$MEMBER_NAME" 'HOST=$MY_POD_IP;PORT=22581';
QUIT;
EOF

gsqlnet sys gliese --as sysdba --dsn=$GLOCATOR_MASTER_NAME<<EOF
