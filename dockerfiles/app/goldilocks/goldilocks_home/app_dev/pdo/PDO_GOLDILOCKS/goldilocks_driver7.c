...
static int goldilocks_handle_closer( pdo_dbh_t *dbh )
{
    pdo_goldilocks_db_handle *H = (pdo_goldilocks_db_handle*)dbh->driver_data;
...
static zend_long goldilocks_handle_doer( pdo_dbh_t  * dbh,
                                         const char * sql,
                                         size_t       sql_len )
{
    pdo_goldilocks_db_handle *H = (pdo_goldilocks_db_handle *)dbh->driver_data;
...
static char *pdo_goldilocks_last_insert_id( pdo_dbh_t  * dbh,
                                            const char * name,
                                            size_t     * len )
{
    pdo_goldilocks_db_handle *H = (pdo_goldilocks_db_handle *)dbh->driver_data;
...
static int goldilocks_handle_begin( pdo_dbh_t *dbh )
{
    pdo_goldilocks_db_handle *H = (pdo_goldilocks_db_handle *)dbh->driver_data;
...
static int goldilocks_handle_commit( pdo_dbh_t *dbh )
{
    pdo_goldilocks_db_handle *H = (pdo_goldilocks_db_handle *)dbh->driver_data;
    SQLRETURN rc;
...
static int goldilocks_handle_rollback( pdo_dbh_t *dbh )
{
    pdo_goldilocks_db_handle *H = (pdo_goldilocks_db_handle *)dbh->driver_data;
    SQLRETURN rc;
        ...
        case PDO_ATTR_CLIENT_VERSION:
            if( H->dbc )
            {
                if( SQL_SUCCEEDED(GDLGetInfo( H->dbc,
                                              ...
                                              NULL )) )
                {
                    ZVAL_STRINGL(val, buf, strlen(buf));
                    return 1;
                }
