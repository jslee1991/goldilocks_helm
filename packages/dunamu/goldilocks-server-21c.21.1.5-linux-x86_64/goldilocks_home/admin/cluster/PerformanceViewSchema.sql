...
SELECT 
     ...
     , CAST( ssenv.VALUE AS NUMBER )                   -- STAT_VALUE
  FROM 
       FIXED_TABLE_SCHEMA.X$SS_SESS_ENV@GLOBAL[IGNORE_INACTIVE_MEMBER] AS ssenv   
  WHERE ssenv.CATEGORY = 0
UNION ALL
SELECT 
...
SELECT 
  ...
  FROM 
       FIXED_TABLE_SCHEMA.X$SS_SYSTEM_INFO@GLOBAL[IGNORE_INACTIVE_MEMBER] AS ssinfo   
...
DROP VIEW IF EXISTS PERFORMANCE_VIEW_SCHEMA.GV$PLAN_HISTORY;
...
CREATE VIEW PERFORMANCE_VIEW_SCHEMA.GV$PLAN_HISTORY
(  
       ORIGIN_MEMBER_NAME
     , DRIVER_MEMBER_POS
     , DRIVER_SESSION_ID
...
SELECT
  ...
  FROM 
       FIXED_TABLE_SCHEMA.X$PLAN_HISTORY@GLOBAL[IGNORE_INACTIVE_MEMBER] AS xhist
;
...
SELECT 
     ...
     , LAST_EXEC_TIME
  FROM 
       PERFORMANCE_VIEW_SCHEMA.GV$PLAN_HISTORY@LOCAL
;
...
GRANT SELECT ON TABLE PERFORMANCE_VIEW_SCHEMA.GV$PLAN_HISTORY TO PUBLIC;
...
DROP PUBLIC SYNONYM IF EXISTS GV$PLAN_HISTORY;
...
DROP VIEW IF EXISTS PERFORMANCE_VIEW_SCHEMA.GV$PLAN_HISTORY_LATEST;
...
CREATE VIEW PERFORMANCE_VIEW_SCHEMA.GV$PLAN_HISTORY_LATEST
(  
       ORIGIN_MEMBER_NAME
     , DRIVER_MEMBER_POS
     , DRIVER_SESSION_ID
...
SELECT 
     ...
     , xhist.LAST_EXEC_TIME                          -- LAST_EXEC_TIME
  FROM 
       FIXED_TABLE_SCHEMA.X$PLAN_HISTORY@LOCAL AS xhist
 WHERE 
...
SELECT
  ...
  FROM
       FIXED_TABLE_SCHEMA.X$PLAN_HISTORY@GLOBAL[IGNORE_INACTIVE_MEMBER] AS xhist
 WHERE 
       ( xhist.DRIVER_MEMBER_POS, xhist.DRIVER_SESSION_ID, xhist.DRIVER_CL_STMT_ID )
             = ( SELECT DRIVER_MEMBER_POS, DRIVER_SESSION_ID, CL_STMT_ID
                   FROM FIXED_TABLE_SCHEMA.X$PLAN_HISTORY@LOCAL
                  WHERE SESSION_ID = SESSION_ID()
                    AND PLAN_HISTORY_POS = 1 )
;
...
GRANT SELECT ON TABLE PERFORMANCE_VIEW_SCHEMA.GV$PLAN_HISTORY_LATEST TO PUBLIC;
...
DROP PUBLIC SYNONYM IF EXISTS GV$PLAN_HISTORY_LATEST;
