...
class TestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.cnxn   = pygoldilocks.connect( 'DSN=GOLDILOCKS;UID=test;PWD=test' )
    ...
    def test_different_bindings(self):
        self.cursor.execute("create table t1(n int)")
        self.cursor.execute("create table t2(d date )")
        self.cursor.execute("insert into t1 values (?)", 1)
    ...
    def test_getinfo_string(self):
        value = self.cnxn.getinfo(pygoldilocks.SQL_CATALOG_NAME_SEPARATOR)
        self.assert_(isinstance(value, str))

    def test_getinfo_bool(self):
        value = self.cnxn.getinfo(pygoldilocks.SQL_ACCESSIBLE_TABLES)
        self.assert_(isinstance(value, bool))

    def test_getinfo_int(self):
        value = self.cnxn.getinfo(pygoldilocks.SQL_DEFAULT_TXN_ISOLATION)
        self.assert_(isinstance(value, (int, long)))

    def test_getinfo_smallint(self):
        value = self.cnxn.getinfo(pygoldilocks.SQL_CONCAT_NULL_BEHAVIOR)
    ...
    def _test_strtype(self, sqltype, value, colsize=None):
        ...
        #print ('_test_strtype func', self, type(v), v, type(value), value)
        if v is not None:
    ...
    def _maketest(value):
        def t(self):
            #print( '_maketest', bytearray(value), value )
    ...
    # long varchar
    #
    def test_long_varchar_fetch(self):
        self.cursor.execute("create table t1( c1 long varchar )")
        self.cursor.execute("insert into t1 values( 'A' )")
        self.cursor.execute("update t1 set c1=c1||c1||c1||c1||c1||c1||c1||c1||c1||c1")
        self.cursor.execute("update t1 set c1=c1||c1||c1||c1||c1||c1||c1||c1||c1||c1")
        self.cursor.execute("update t1 set c1=c1||c1||c1||c1||c1||c1||c1||c1||c1||c1")
        self.cursor.execute("update t1 set c1=c1||c1||c1||c1||c1||c1||c1||c1||c1||c1")
        self.cursor.execute("update t1 set c1=c1||c1||c1||c1||c1||c1||c1||c1||c1||c1")
        ...
        self.cursor.execute("select c1 from t1" )
        row = self.cursor.fetchval()

        self.assertEqual( len(row), 1000000 )
    ...
    def test_unicode_query(self):
        self.cursor.execute(u"select 1 from dual")
    ...
    # integer, so pygoldilocks can't recognize it and map it back to True/False.
    #
...
#        self.assertEqual(type(v), Decimal)
    ...
    def test_close_cnxn(self):
        ...
        self.assertRaises(pygoldilocks.ProgrammingError, self._exec)
    ...
    def test_version(self):
        self.assertEquals(3, len(pygoldilocks.version.split('.'))) # 1.3.1 etc.
    ...
    def test_rowcount_select(self):
        ...
        pygoldilocks calls SQLRowCount after each execute and sets Cursor.rowcount.  Databases can return the actual rowcount
    ...
    def test_lower_case(self):
        "Ensure pygoldilocks.lowercase forces returned column names to lowercase."

        # Has to be set before creating the cursor, so we must recreate self.cursor.

        pygoldilocks.lowercase = True
        self.cursor = self.cnxn.cursor()
        ...
        # Put it back so other tests dont fail.
        pygoldilocks.lowercase = False
    ...
    def test_executemany_non_alphabet(self):
        self.cursor.execute("create table t1(a int, b varchar(10))")

        self.cursor.fast_executemany = False
        
        params = [ (1, str('갉')), (2, str('낣')), (3, str('뷃')) ]
    ...
    def test_fast_executemany_non_alphabet(self):
        self.cursor.execute("create table t1(a int, b varchar(10))")

        self.cursor.fast_executemany = True
        
        params = [ (1, u'갉'), (2, u'낣'), (3, u'뷃') ]
    ...
    def test_executemany_failure(self):
        ...
        self.failUnlessRaises(pygoldilocks.Error, self.cursor.executemany, "insert into t1(a, b) value (?, ?)", params)
    ...
    def test_autocommit(self):
        ...
        othercnxn = pygoldilocks.connect(self.connection_string, autocommit=True)
...
def main():
    ...
    suite = load_tests(TestCase, options.test, connection_string)
